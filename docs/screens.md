...
- URLクエリの `token` を読み取り、フォーム(名前、パスワード、確認用パスワード)を表示する
- 登録ボタン → `POST /api/invitations/accept`。トークンが不正・期限切れ・使用済み(`400 TOKEN_INVALID`)のときはフォームの代わりにエラーメッセージを表示する
- 登録完了後はC-04(ログイン)へ誘導します。**自動ログインはしません**(登録とログインを分けてシンプルに保つため)

### A-04: メニュー管理

- メニュー一覧テーブル(`GET /api/menus?include_inactive=true`。名前・所要時間・価格・有効/無効・並び順)
- 作成・編集フォーム(名前、説明、所要時間、価格、有効/無効、並び順。モーダルでも別画面でも可)
- 並び順の変更は `sort_order` の**数値を直接編集する形で構いません**(ドラッグ&ドロップは不要)
- 削除ボタン: `409 MENU_IN_USE` のときは「予約実績があるため無効化で対応してください」と案内する

### A-05: スタッフ管理

- 招待発行フォーム(メールアドレス、ロール選択)→ `POST /api/invitations`
- 招待一覧(`GET /api/invitations`)。列: email / role / 状態(未使用・使用済み・期限切れ)
- スタッフ一覧(`GET /api/staff?include_inactive=true`)と、スタッフごとのプロフィール編集フォーム(表示名、自己紹介、画像URL、有効/無効)→ `PATCH /api/staff/:id`
- 対応メニューのチェックボックス(メニュー一覧に対するチェック)→ `PUT /api/staff/:id/menus`
- プロフィールと対応メニューは**別々の保存ボタンで構いません**(APIが別のため。1フォームに統合する必要はありません)
...
- API呼び出しはすべて `credentials: "include"` を付け、401のときはC-04へリダイレクトします。ただしこのルールの適用範囲は**保護画面・保護操作のみ**です。アプリ起動時の `GET /api/auth/me` が401になるのは単なる「未ログイン状態」なので、公開画面(C-01、C-05、C-06など)ではリダイレクトせずそのまま表示を続けます。
