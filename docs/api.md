...
| `SHIFT_EXCEPTION_EXISTS` | 409 | 同一スタッフ・同一日のシフト例外が既に存在する |
...
成功時のステータスコードとID指定の扱いは、全エンドポイントで次に統一します。

| ケース | ステータス |
|---|---|
| POST成功(リソース作成) | `201 Created` |
| PATCH・PUT成功 | `200 OK`(更新後のリソースを返す) |
| DELETE成功 | `204 No Content`(ボディなし) |
| 存在しないIDの指定 | `404 NOT_FOUND` |
| 数値でないID(`/api/menus/abc` など) | `400 VALIDATION_ERROR` |

例外は `DELETE /api/reservations/:id`(キャンセル)です。物理削除ではなくステータス変更のため、更新後の予約を `200 OK` で返します。
...
| メソッド | パス | 認証 | ロール | 概要 |
|---|---|---|---|---|
| GET | `/api/health` | 不要 | — | 死活監視用。`{"status": "ok"}` を返す |
...
| POST | `/api/auth/verify-email` | 不要 | — | メール確認を完了し `email_verified_at` を設定する(body: `{"token": "..."}`) |
...
- メール確認は**GETではなくPOST**です。確認メールのリンクは画面C-03(`/verify-email?token=...`)を開き、画面がクエリのトークンを使って `POST /api/auth/verify-email` を**1回だけ**呼びます。GETでトークンを消費する設計は、React StrictMode等による二重リクエストで2回目が `400 TOKEN_INVALID` になり自滅するため採用しません。
- ログインのエラー優先順位: パスワードが一致しない場合は常に `401 INVALID_CREDENTIALS` を返します。`403 EMAIL_NOT_VERIFIED` を返すのは**パスワードが一致した場合のみ**です(アカウントの状態を第三者に漏らさないため)。
...
- 既に登録済みのemailへの招待発行は `409 EMAIL_TAKEN` です。accept時点でそのemailが登録済みになっていた場合も `409 EMAIL_TAKEN` を返し、**トークンは消費しません**(`used_at` は記録しない)。
- 同一emailへの再発行は許可します(旧トークンも期限内であれば有効のままです)。
...
- DELETEで削除をブロックするのは**予約実績(reservation_menusに行がある)場合のみ**です。`staff_menus` の紐付けはブロック理由にせず、削除と同一トランザクション内で自動解除します。
...
GET `/api/menus`・`/api/staff` のように「未ログインでも呼べるが、ロールで応答が変わる」エンドポイントは次のルールに統一します。

- 公開エンドポイントに無効・期限切れのCookieが付いていても `401` にせず、**匿名として扱います**(公開画面がログイン切れで壊れないようにするため)。
- `?include_inactive=true` が有効なのは**admin/staffのみ**です。匿名・customerが付けた場合はエラーにせず、無視して有効分のみ返します。
- `GET /api/staff/:id` は、`active = false` のスタッフを匿名・customerには `404 NOT_FOUND` として返します(admin/staffは取得可)。
- `GET /api/staff` の並び順は `user_id` 昇順です。
- `PATCH /api/staff/:id` を実行できるのはadminまたは本人staffです。ただし `active` の変更はadminのみで、本人staffがリクエストに `active` を含めた場合は `403 FORBIDDEN` を返します。
...
| POST | `/api/shifts/exceptions` | 要 | staff(本人) / admin | シフト例外の作成。同一(staff_id, date)が既にあれば `409 SHIFT_EXCEPTION_EXISTS` |
...
システムが送信するメールは次の2種類です(リマインダーS-1を除く)。リンク先はフロントエンドの画面URLで、APIのURLではありません。

| 種別 | トリガー | 宛先 | 件名例 | リンク先 |
|---|---|---|---|---|
| メール確認 | `POST /api/auth/signup` | 登録されたメールアドレス | 【Lumina Reserve】メールアドレスの確認 | C-03 `/verify-email?token=...` |
| 招待 | `POST /api/invitations` | 招待先メールアドレス | 【Lumina Reserve】スタッフ招待のご案内 | A-01 `/invitations/accept?token=...` |

## M1系APIの入出力例

認証・招待・メニュー・スタッフの代表的なリクエスト/レスポンス例です(availabilityほどの詳細は不要な、形合わせのための最小例)。

### POST /api/auth/signup

```json
{ "name": "山田 花子", "email": "hanako@example.com", "password": "password123" }
```

レスポンス `201 Created`:

```json
{ "id": 10, "name": "山田 花子", "email": "hanako@example.com", "role": "customer" }
```
...
レスポンス `200 OK`(`lumina_session` Cookieの発行と同時に返す):
...
```json
{ "email": "aoi@example.com", "role": "staff" }
```

レスポンス `201 Created`。**`token` はレスポンスに含めません**(招待メール経由でのみ受け渡します):

```json
{ "id": 3, "email": "aoi@example.com", "role": "staff", "expires_at": "2026-07-18T10:00:00+09:00" }
```

### GET /api/invitations

レスポンス `200 OK`(1要素例。こちらも `token` は返しません):

```json
[
  { "id": 3, "email": "aoi@example.com", "role": "staff", "expires_at": "2026-07-18T10:00:00+09:00", "used_at": null }
]
```
...
```json
{ "token": "b7d2e8...", "name": "青井 葵", "password": "password123" }
```

レスポンス `201 Created`:

```json
{ "id": 3, "name": "青井 葵", "email": "aoi@example.com", "role": "staff" }
```
...
```json
[
  { "id": 1, "name": "カット", "description": "シャンプー・ブロー込み", "duration_min": 60, "price": 4950, "active": true, "sort_order": 1 }
]
```
...
```json
[
  { "id": 3, "display_name": "AOI", "bio": "得意スタイルはショートです", "image_url": null, "menu_ids": [1, 2, 4] }
]
```

### PUT /api/staff/:id/menus

```json
{ "menu_ids": [1, 2] }
```

レスポンス `200 OK`(更新後の `menu_ids` を含むスタッフを返す)。
...
- 簡易登録した顧客はランダムパスワードのcustomerとして作成します(`email_verified_at` はNULLのまま)。発展課題としてパスワード再設定を実装する場合は、再設定メールのリンクを踏んだ時点でメール所有が確認できるため、再設定完了時に `email_verified_at` も設定します(NULLのままでは `403 EMAIL_NOT_VERIFIED` でログインできないため)。
