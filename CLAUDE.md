# CLAUDE.md — Lumina Reserve プロジェクトガイド
...
- 空き枠計算(M2-04)と二重予約防止(M3-01)は `.claude/skills/reservation-domain` を読んでから実装する。
- 受入条件に「テストで検証」とある項目は、テストなしで完了扱いにしない。
- 小さな依存ライブラリの追加(バリデーション、日付処理、パスワードハッシュなど)は事前確認なしで行ってよい。ただし追加理由を1行、PRに書く。
- 環境起因のエラー(pnpm、Prisma、Docker、StrictModeなど)に遭遇したら、まず `docs/tips.md`(環境つまずき集)に同じ症状がないか確認する。
...
| スキル | 使いどころ |
|---|---|
| `.claude/skills/project-onboarding` | プロジェクト開始時・文脈を失ったとき。全体像の再取得 |
| `.claude/skills/setup-github-project` | 最初の1回。label / milestone / issueを受講者のリポジトリへ複製 |
| `.claude/skills/issue-workflow` | 各issueの着手時。読解→仕様特定→計画→実装→テスト→PRの型 |
| `.claude/skills/spec-compliance` | PR作成前。docsとの突合チェックリストを出力 |
| `.claude/skills/reservation-domain` | M2-04・M3-01と、空き枠/予約に関わるすべての実装 |
